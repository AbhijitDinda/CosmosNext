...
import { Plus, Clock, MinusCircle, ChevronUp, ChevronDown, StarIcon } from "lucide-react";
...
const TestList = ({ cosmos_tests = [], expert_rating_tests = [] }) => {
  ...
  ], [cosmos_tests, expert_rating_tests]);
  ...
  const filteredTests = useMemo(() => availableTests.reduce((acc, curr) => {
    acc.push(...curr.data);
    return acc;
  }, []).filter((test) => test.test_name.toLowerCase().includes(searchQuery.toLowerCase())), [availableTests, searchQuery]);
    ...
    <ErrorBoundary>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-1 font-OpenSans">
        ...
        <Card className="py-5 md:py-8 lg:py-10 px-2 lg:px-5 rounded-r-sm rounded-l-none">
          ...
          {/* Add Adaptive Assessment Checkbox and AI Button */}
