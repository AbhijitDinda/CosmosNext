...
export const editQuestionInTeamInventory = async ({ token , logout, post_data, q_id}) => {
    try {
        console.log("inside post_data",post_data,logout)
        const response = await axios.post(`/admin/team-inventory/questions/update/${q_id}`,post_data , {
            headers: {
                'Authorization': `Bearer ${token}`,
                'Accept': 'application/json'
            }
...
export const editSubQuestionInTeamInventory = async ({ token , logout, post_data, SubQ_id}) => {
    try {
        console.log("inside post_data",post_data,logout)
        const response = await axios.post(`/admin/team-inventory/sub-questions/update/${SubQ_id}`,post_data , {
            headers: {
                'Authorization': `Bearer ${token}`,
                'Accept': 'application/json'
            }
...
export const editTraitInTeamInventory = async ({ token , logout, post_data, traits_id}) => {
    try {
        console.log("inside post_data",post_data,logout)
        const response = await axios.post(`/admin/team-inventory/traits/update/${traits_id}`,post_data , {
            headers: {
                'Authorization': `Bearer ${token}`,
                'Accept': 'application/json'
            }
